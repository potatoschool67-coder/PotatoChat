      ...
      case 'potatobot':
        try {
          if (args === 'start') {
            ...
            response = '🚀 Starting Ollama server...';
            break;
          }
          ...
          if (args === 'stop') {
            ...
            response = '🛑 Stopping Ollama server...';
            break;
          }
          ...
          if (ollamaRes.ok) {
            ...
            if (args) {
              ...
              if (availableModels.includes(args)) {
                await fetch('http://localhost:3000/api/ollama/model', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ model: args }),
                });
                response = `✅ Switched to model: ${args}`;
              } else {
                response = `Model "${args}" not found. Available: ${models}`;
              }
            } else {
              let currentModel = 'gemma4:31b-cloud';
              try {
                const settingsRes = await fetch('http://localhost:3000/api/ollama/model');
                if (settingsRes.ok) {
                  const settingsData = await settingsRes.json();
                  currentModel = settingsData.model || currentModel;
                }
              } catch (e) {}
              response = `✅ Ollama is running! Current model: ${currentModel}\nAvailable models: ${models}`;
            }
          } else {
            response = '❌ Ollama API error - Ollama may not be running. Use /ollama start to start it.';
          }
        } catch (e) {
          response = '❌ Ollama is not running. Use /ollama start to start it.';
        }
        break;
      case 'servers':
